	...
	if( $resultado->num_rows > 0 ){
		...
		if( sha1( $usuario['idusuario'] === $idusuario ) ){
			if( $password1 === $password2 ){
				if($tipo=="prof"){
					$sql = "UPDATE Profesor SET password = '".sha1($password1)."' WHERE ID_prof = ".$usuario['idusuario'];
				}else if($tipo=="alumno"){
					$sql = "UPDATE ALumno SET password = '".sha1($password1)."' WHERE ID_alumno = ".$usuario['idusuario'];
				}
